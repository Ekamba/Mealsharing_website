...
router.get("/", (req, res) => {
  connection.query(
    `
    select meal.id, meal.title, meal.location, meal.max_reservations, sum(coalesce(reservations.number_of_guests, 0)) as total_reservations
from meal
left join reservations 
on meal.id = reservations.meal_id
group by meal.id; `,
    (err, results, fields) => {
